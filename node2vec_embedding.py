...
class node2vec:
    ...
    def learning_features(self):
        ...
        for node in nodes:
            result=f[node]
            result_arry[node]=result
        #res = [f[x] for x in nodes]
        return result_arry

def les_miserables_graph(ppi_line_csv):
    ppi_line_arry = np.array(pd.read_csv(ppi_line_csv))
    node1_list = ppi_line_arry[0:, 0]
    node2_list = ppi_line_arry[0:, 1]
    weight_list=ppi_line_arry[0:, 2]

    G = nx.Graph()
    for index in range(0,len(ppi_line_arry)):
        G.add_edge(node1_list[index], node2_list[index], weight=weight_list[index])
    return G

def write_embedding_ht(embeddings_ht, npy_result_path):
    for access in embeddings_ht.keys():
        embedding = embeddings_ht[access]
        np_file = npy_result_path + "/" + access + "_net-feature.npy"
        np.save(np_file, embedding)
        print("write npy ",access)
